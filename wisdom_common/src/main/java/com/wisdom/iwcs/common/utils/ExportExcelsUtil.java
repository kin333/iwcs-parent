...
public class ExportExcelsUtil {
    ...
    public static <T> void exportExcel(List<String> headers, List<String> columns, List<T> dataset, OutputStream out) {
        exportExcel(headers, columns, dataset, out, "yyyy-MM-dd");
    }
    ...
    public static <T> void exportExcel(List<String> headers, List<String> columns, List<T> dataset, OutputStream out, String pattern) {
        exportExcel(0, 0, 0, "sheet1", headers, columns, dataset, out, pattern);
    }
     ...
     * @param dataset 1.需要显示的数据集合,集合中一定要放置符合javabean风格的类的对象。此方法支持的 javabean属性的数据类型有基本数据类型及String,Date,byte[](图片数据)
     *                2.或者Map,key是string类型，value属性
     * @param out     与输出设备关联的流对象，可以将EXCEL文档导出到本地文件或者网络中
     * @param pattern 如果有时间数据，设定输出格式。默认为"yyy-MM-dd"，目前不需要
     */
            ...
            for (int index = 0; index < dataList.size(); index++) {
                ...
                for (String column : columns) {
                    ...
                    if (value instanceof byte[]) {
                        ...
                        row.setHeightInPoints(600);
                        // 设置图片所在列宽度为80px,注意这里单位的一个换算
                        sheet.setColumnWidth(columnIdx, (short) (35.7 * 400));
                        // sheet.autoSizeColumn(i);
                        byte[] bsValue = (byte[]) value;
