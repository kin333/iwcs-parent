        ...
        super(RabbitMQConstants.NODE_ACTION_QUEUE,
                ...
                consumerActionInfo -> {
                    ...
                    for (int i = 0; i < 7; i++) {
                        ...
                        //如果RabbitMQ的速度快于Action数据的插入速度,会造成这里查不到,延时1秒后再次查询
                        try {
                            Thread.sleep(1000);
                    ...
                    if (StringUtils.isNotBlank(subTaskAction.getPreActions())) {
                        ...
                        if (preSubTaskAction == null) {
                            //前置请求未生成
                            logger.info("子任务号{},前置请求{}不存在", subTaskAction.getSubTaskNum(), subTaskAction.getPreActions());
                            return;
                        }
                    ...
                    tmpTaskAction.setLastExecTime(new Date());
                    tmpTaskAction.setResendCount(subTaskAction.getResendCount() + 1);
