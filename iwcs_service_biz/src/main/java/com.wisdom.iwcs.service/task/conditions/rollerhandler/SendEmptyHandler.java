 ...
 */
@Component
public class SendEmptyHandler implements IConditionHandler {
    ...
    public boolean handleCondition(SubTaskCondition subTaskCondition) {
        ...
        logger.info("子任务{}送空料箱data生成结束,生成data为:{}", subTaskCondition.getSubTaskNum(), jsonString);
    ...
    public boolean rollbackCondition(SubTaskCondition subTaskCondition) {
        ...
        if (rows <= 0) {
            logger.info("子任务{}送空料箱前置条件回滚失败", subTaskCondition.getSubTaskNum());
            return false;
        }
        logger.info("子任务{}送空料箱前置条件回滚成功", subTaskCondition.getSubTaskNum());
        return true;
    }
}