 ...
 */
@Service
public class CheckEleCanLeaveHandler implements IConditionHandler {
    private static final Logger logger = LoggerFactory.getLogger(CheckEleCanLeaveHandler.class);
    @Autowired
    SubTaskConditionMapper subTaskConditionMapper;

    @Override
    public boolean handleCondition(SubTaskCondition subTaskCondition) {
        logger.info("子任务{}开始检查电梯是否可以离开", subTaskCondition.getSubTaskNum());
        subTaskCondition.setConditonHandler("checkEleCanLeaveHandler");
        while (true) {
            ...
            if (tmpSubTaskCondition != null && TaskConstants.metStatus.CONFORM.equals(tmpSubTaskCondition.getConditionMetStatus())) {
                logger.info("子任务{}检查电梯已可以离开",  subTaskCondition.getSubTaskNum());
                break;
            }
            try {
                Thread.sleep(5000L);
