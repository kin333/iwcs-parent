...
import static com.wisdom.iwcs.common.utils.InspurBizConstants.InStock.HAVE_GOODS_INT;

/**
 * 后置条件检查,检查任务是否完成,并且将货架设置为满货
 */
@Service
public class CheckAndFullPodHandler implements IConditionHandler {
    @Autowired
    ...
    public boolean handleCondition(SubTaskCondition subTaskCondition) {
        if (checkWorkStatusHandler.handleCondition(subTaskCondition)) {
            ...
            basePodDetailMapper.updateInStock(subTask.getPodCode(), HAVE_GOODS_INT);
            return true;
        }
        return false;
    }
